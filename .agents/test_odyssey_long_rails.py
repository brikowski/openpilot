...
class TestOdysseyLongRails(unittest.TestCase):
  ...
  def test_uphill_gas_load_tapers_at_zero_and_keeps_raw_accel(self):
    ...
    near_zero = np.tile([0.01, -0.01], 100)
    _, uphill_crossings = _run(True, near_zero, pitch=0.03, vego=31.0)
    crossing_gas = np.array([gas for _, gas, _ in uphill_crossings])
  ...
  def test_odyssey_lateral_authority_uses_stock_range(self):
    """Guard the stock Odyssey steering map after retiring the 3840 road candidate."""
    CP = _car_params()
