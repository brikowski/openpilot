...
def test_direct_steering_matches_following_caroutput_and_counts_extended_commands():
  sent = np.array([
    [0.00, 3000, 1, 0],
    [0.01, 3840, 1, 1],
    [0.02, 0, 0, 2],
    [1.00, 3100, 1, 3],  # no nearby carOutput: do not cross a segment gap
  ])
  output_t = np.array([0.005, 0.015, 0.025, 1.20])
  output = np.array([3000, 3800, 0, 3100])
  metrics = steering_direct_metrics(sent, output_t, output, stock_cap=2560)
  assert metrics["lat_direct_sent_frames"] == 4
  assert metrics["lat_direct_active_frames"] == 3
  assert metrics["lat_direct_sent_max_abs"] == 3840
  assert metrics["lat_direct_extended_frames"] == 3
  assert metrics["lat_direct_paired_frames"] == 3
  assert np.isclose(metrics["lat_direct_exact_frac"], 2 / 3)
  assert np.isclose(metrics["lat_direct_mae"], 40 / 3)
  assert np.isclose(metrics["lat_direct_delay_ms_median"], 5.0)


def test_direct_steering_decoder_uses_bus_one_sendcan_only():
  from opendbc.can.packer import CANPacker

  packer = CANPacker(validate_log.ODYSSEY_PT_DBC)
  direct = packer.make_can_msg("STEERING_CONTROL", 1,
                               {"STEER_TORQUE": 3000, "STEER_TORQUE_REQUEST": 1, "COUNTER": 2})
  radar = packer.make_can_msg("STEERING_CONTROL", 0,
                              {"STEER_TORQUE": 2000, "STEER_TORQUE_REQUEST": 1, "COUNTER": 3})
  msg = SimpleNamespace(
    logMonoTime=1_000_000_000,
    sendcan=[SimpleNamespace(address=address, dat=data, src=bus) for address, data, bus in (radar, direct)],
    which=lambda: "sendcan",
  )
  decoded = validate_log._steering_direct_series([msg])
  assert decoded.shape == (1, 4)
  assert decoded[0, 1] == 3000
  assert decoded[0, 2] == 1
  assert decoded[0, 3] == 2
...
def test_domain_model_selects_exact_opendbc_source_semantics():
  ...
  assert _brake_passthrough_expected("d50a3a4843ed")
...
def test_uphill_near_zero_gas_steps_exclude_domain_edges_and_flat_terrain():
  grid = np.arange(0.0, 1.0, 0.01)
  requested = np.full(len(grid), 0.01)
  requested[20:40] = -0.01
  gas = np.full(len(grid), 180.0)
  gas[20:40] = 450.0
  gas[60:80] = -30000.0  # Domain exit and re-entry are not live-gas steps.
  brake_request = np.zeros(len(grid), dtype=bool)
  brake_request[90:] = True
  gas[90:] = 600.0  # A brake-domain jump is not this symptom either.

  def measure(pitch):
    return uphill_near_zero_gas_step_metrics(
      grid, requested, np.ones(len(grid), dtype=bool), np.full(len(grid), 20.0),
      np.full(len(grid), pitch), brake_request, np.zeros(len(grid), dtype=bool), gas,
      speed_min=10.0, pitch_min=0.02, pitch_max=0.06,
      request_abs_max=0.03, request_delta_max=0.05, gas_delta_min=150.0,
    )

  uphill = measure(0.03)
  assert uphill["uphill_near_zero_gas_step_events"] == 2
...
def test_uphill_tracking_bins_separate_demand_and_exclude_wrong_domains():
  n = 1000
  grid = np.arange(n, dtype=float) * 0.01
  requested = np.r_[np.full(400, 0.30), np.full(400, 0.90), np.full(200, 0.30)]
  actual = requested + np.r_[np.full(400, -0.20), np.full(400, 0.40), np.zeros(200)]
  wire = requested.copy()
  speed = np.full(n, 20.0)
  pitch = np.full(n, 0.06)
  on = np.ones(n, dtype=bool)
  off = np.zeros(n, dtype=bool)
  gas = np.full(n, 500.0)

  def measure(*, lead=off, brake=off, grade=pitch, live_gas=gas):
    return uphill_tracking_bin_metrics(
      grid, requested, actual, wire, speed, grade, on, on, on, lead,
      off, off, brake, live_gas,
    )

  result = measure()
  assert result["uphill_moderate_episodes"] == 2
  assert np.isclose(result["uphill_moderate_sec"], 5.0)
  assert np.isclose(result["uphill_moderate_response_error_median"], -0.20)
  assert result["uphill_high_episodes"] == 1
  assert np.isclose(result["uphill_high_sec"], 3.5)
  assert np.isclose(result["uphill_high_response_error_median"], 0.40)
  assert np.isclose(result["uphill_high_wire_rms"], 0.0)
  for mutation in ({"lead": on}, {"brake": on}, {"grade": np.zeros(n)},
                   {"live_gas": np.full(n, -30000.0)}):
    masked = measure(**mutation)
    assert masked["uphill_moderate_episodes"] == 0
    assert masked["uphill_high_episodes"] == 0


def test_uphill_tracking_bin_does_not_join_retained_log_gaps():
  grid = np.r_[np.arange(200) * 0.01, 5.0 + np.arange(200) * 0.01]
  n = len(grid)
  on = np.ones(n, dtype=bool)
  off = np.zeros(n, dtype=bool)
  result = uphill_tracking_bin_metrics(
    grid, np.full(n, 0.30), np.full(n, 0.10), np.full(n, 0.30),
    np.full(n, 20.0), np.full(n, 0.06), on, on, on, off,
    off, off, off, np.full(n, 500.0),
  )
  assert result["uphill_moderate_episodes"] == 2
  assert np.isclose(result["uphill_moderate_sec"], 3.0)
