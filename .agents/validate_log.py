...
# Sunnypilot v2026.003 parents vendored the older tinygrad model instead of carrying the upstream
# ONNX path.  The manifest is the immutable pointer to the two model chunks in that parent tree.
# Keep this separate from MODEL_PATHS so current OpenPilot parents continue to resolve their ONNX
# blobs exactly as before.
MODEL_FALLBACK_PATHS = {
  "small": "openpilot/selfdrive/modeld/models/driving_tinygrad.pkl.chunkmanifest",
}
...
UNDER_SET_SPEED_GAP_MIN = 0.5 * 0.44704  # m/s: diagnostic lower edge (0.5 mph below set)
UNDER_SET_SPEED_GAP_MAX = 1.5 * 0.44704  # m/s: diagnostic upper edge (1.5 mph below set)
UNDER_SET_SPEED_REQUEST_MIN = 0.05       # m/s^2: ignore near-zero request noise
UNDER_SET_SPEED_MIN_EPISODE_S = 2.0      # require a sustained hold, not a transition
LEVEL_RESPONSE_PITCH_MAX = 0.02          # rad: near-level exposure readout only
LEVEL_RESPONSE_REQUEST_MIN = 0.10        # m/s^2: material positive request for the readout
LEVEL_RESPONSE_MIN_EPISODE_S = 2.0       # require a sustained hold, not a transition
...
# Diagnostic exposure from the 2026-09-19 uphill gas-pulse screen; not a comfort limit.
UPHILL_GAS_STEP_SPEED_MIN = 10.0
UPHILL_GAS_STEP_PITCH_MIN = 0.02
UPHILL_GAS_STEP_PITCH_MAX = 0.06
UPHILL_GAS_STEP_REQUEST_ABS_MAX = 0.03
UPHILL_GAS_STEP_REQUEST_DELTA_MAX = 0.05
UPHILL_GAS_STEP_COMMAND_DELTA_MIN = 150.0
...
THREE_DOMAIN_ROAD_BRAKE_ENTRY_BY_COMMIT = {
  ...
  "359f3574d67f": -0.30,  # lateral-only trial; Odyssey brake-domain selection unchanged
  "69a81e7da7f": -0.30,  # removes uphill gas load; brake-domain selection unchanged
  "d50a3a4843ed": -0.30,  # request-ramped uphill gas load; brake-domain selection unchanged
}
RAW_DOMAIN_COMMITS = {
  "f6e4f07bdc61",  # ody-op-test2 fresh brake-source reset
  "44f2987cb6ed",  # upstream stock comparison routes
  "06743dfb39cf",  # upstream opendbc pinned by the historical Sunnypilot v2026.003 vendor tree
}
THREE_DOMAIN_COMMITS = {
  ...
  "359f3574d67f",  # lateral-only child; raw three-domain output is unchanged
  "69a81e7da7f",  # retires uphill GAS_COMMAND load; raw three-domain output is unchanged
  "d50a3a4843ed",  # request-ramped uphill GAS_COMMAND; raw three-domain output is unchanged
}
...
def _cruise_button_series(msgs):
  """Return raw cruise button edges from carState without interpreting driver intent."""
  times, types, pressed = [], [], []
  for m in msgs:
    if m.which() != "carState":
      continue
    state = _safe_attr(m, "carState")
    events = _safe_attr(state, "buttonEvents", ()) or ()
    try:
      for event in events:
        times.append(m.logMonoTime / 1e9)
        types.append(str(_safe_attr(event, "type", "unknown")))
        pressed.append(bool(_safe_attr(event, "pressed", False)))
    except (TypeError, ValueError):
      # An older/partial schema should not make the route fail validation. The missing stream is
      # represented by empty arrays and the command/response metrics remain valid.
      continue
  return np.asarray(times, dtype=float), np.asarray(types, dtype=object), np.asarray(pressed, dtype=bool)
...
def _steering_direct_series(msgs):
  """Decode Bosch Alpha Long steering sent directly to the powertrain bus."""
  from opendbc.can.parser import CANParser

  parser = CANParser(ODYSSEY_PT_DBC, [("STEERING_CONTROL", 0)], 1)
  sent = []
  for msg in msgs:
    if msg.which() != "sendcan":
      continue
    frames = [(frame.address, frame.dat, frame.src) for frame in msg.sendcan
              if frame.address == STEERING_CONTROL and frame.src == 1]
    if frames and STEERING_CONTROL in parser.update([(msg.logMonoTime, frames)]):
      values = parser.vl["STEERING_CONTROL"]
      sent.append((msg.logMonoTime / 1e9, values["STEER_TORQUE"],
                   values["STEER_TORQUE_REQUEST"], values["COUNTER"]))
  return np.asarray(sent, dtype=float)
...
def _git_commit_message(commit):
  """Read a local commit message for provenance formats that encode a source parent in prose."""
  if not commit:
    return ""
  repo = Path(__file__).resolve().parents[1]
  try:
    out = subprocess.run(["git", "-C", str(repo), "show", "-s", "--format=%B", commit],
                         capture_output=True, text=True, timeout=10)
  except (OSError, subprocess.SubprocessError):
    return ""
...
def _sunnypilot_vendor_opendbc_pointer(parent_commit):
  """Resolve a vendored Sunnypilot parent to the upstream opendbc commit it was built from.

  Sunnypilot v2026.003 stored ``opendbc_repo`` as a regular tree.  Its generated release commit
  message records the upstream OpenPilot base; that base's gitlink is the exact opendbc source.
  Refuse this fallback for an arbitrary tree or message so a prose mention cannot create false
  precision.  The caller still requires a clean initData source.
  """
  entry = _git_tree_entries(parent_commit, ("opendbc_repo",)).get("opendbc_repo")
  if not entry or entry[:2] != ("040000", "tree"):
    return None
  message = _git_commit_message(parent_commit)
  if not re.search(r"(?im)^\s*sunnypilot\s+v[^\r\n]*$", message):
    return None
  match = re.search(r"(?im)^\s*master\s+commit:\s*([0-9a-f]{40})\s*$", message)
  if not match:
    return None
  base_entry = _git_tree_entries(match.group(1), ("opendbc_repo",)).get("opendbc_repo")
  if base_entry and base_entry[:2] == ("160000", "commit") and _full_sha(base_entry[2]):
    return base_entry[2]
  return None
...
def _opendbc_pointer_full(parent_commit, dirty):
  ...
  Current OpenPilot parents use a gitlink directly.  The historical Sunnypilot vendor format is
  resolved through its recorded upstream base by _sunnypilot_vendor_opendbc_pointer().
  """
  ...
  if not parent_commit or dirty is not False:
    return None
  entry = _git_tree_entries(parent_commit, ("opendbc_repo",)).get("opendbc_repo")
  if entry and entry[0] == "160000" and entry[1] == "commit" and _full_sha(entry[2]):
    return entry[2]
  return _sunnypilot_vendor_opendbc_pointer(parent_commit)
...
def _model_provenance(msgs, parent_entries):
  ...
  if small_entry is None:
    small_entry = parent_entries.get(MODEL_FALLBACK_PATHS["small"])
...
def _provenance(msgs):
  ...
  for m in msgs:
    if m.which() == "initData":
      ...
      source_entries = _git_tree_entries(
        commit_full, ("opendbc_repo", *MODEL_PATHS.values(), *MODEL_FALLBACK_PATHS.values())) \
        if dirty is False else {}
      opendbc_full = _opendbc_pointer_full(commit_full, dirty)
...
def analyze(msgs, platform, alpha_longitudinal=None):
  ...
  _, cc_set_speed = _series(msgs, "carControl", lambda m: m.carControl.hudControl.setSpeed)
  button_t, button_types, button_pressed = _cruise_button_series(msgs)
  r.update(cruise_input_metrics(
    button_t, button_types, button_pressed, t_cc, cc_set_speed,
    route_start=t_cc[0] if len(t_cc) else None,
  ))
  _, cc_speed_visible = _series(msgs, "carControl",
                                lambda m: 1.0 if m.carControl.hudControl.speedVisible else 0.0)
  ...
  _, lp_cruise = _series(
    msgs, "longitudinalPlan",
    lambda m: 1.0 if getattr(m.longitudinalPlan.longitudinalPlanSource, "raw", -1) == 0 else 0.0,
  )
  _, lp_allow_throttle = _series(msgs, "longitudinalPlan",
                                 lambda m: 1.0 if m.longitudinalPlan.allowThrottle else 0.0)
  _, lp_has_lead = _series(msgs, "longitudinalPlan",
                           lambda m: 1.0 if m.longitudinalPlan.hasLead else 0.0)
  ...
  # controlsd consumes the latest published plan through SubMaster. Hold aTarget exactly; linear
  # interpolation invents an upstream command between 20 Hz plan messages and masks transitions.
  atarget = _hold_last(grid, t_lp, lp_atarget) if len(t_lp) else cc_accel.copy()
  if len(t_lp):
    cruise_plan = _hold_last(grid, t_lp, lp_cruise) > 0.5
    allow_throttle = _hold_last(grid, t_lp, lp_allow_throttle) > 0.5
    has_lead = _hold_last(grid, t_lp, lp_has_lead) > 0.5
  else:
    cruise_plan = allow_throttle = has_lead = np.zeros(len(grid), dtype=bool)
  ...
  if platform == ODYSSEY and not r["qlog_fallback"] and alpha_longitudinal is True:
    direct_sent = _steering_direct_series(msgs)
  else:
    direct_sent = np.empty((0, 4), dtype=float)
  r.update(steering_direct_metrics(
    direct_sent, t_co, co_lat_torque_can, stock_cap=ODYSSEY_STOCK_LKA_MAX,
  ))

  if platform == ODYSSEY and not r["qlog_fallback"] and alpha_longitudinal is not True:
  ...
  if is_ody:
    # Diagnostic only: isolate sustained, no-lead cruise speed error while the upstream planner
    # allows throttle. This makes the level-road follow-up reproducible without turning a grade
    # response into an opendbc verdict.
    r.update(under_set_speed_metrics(
      grid, cc_set_speed, vego, cc_accel, aego, cc_pitch, active,
      cc_speed_visible > 0.5, cruise_plan, allow_throttle, has_lead,
      gaspressed, brakepressed,
      speed_min=FOLLOW_MIN_VEGO,
      gap_min=UNDER_SET_SPEED_GAP_MIN,
      gap_max=UNDER_SET_SPEED_GAP_MAX,
      request_min=UNDER_SET_SPEED_REQUEST_MIN,
      min_episode_s=UNDER_SET_SPEED_MIN_EPISODE_S,
    ))
...
def _following(msgs, grid, requested, active, pid, pitch, vego, gaspressed, brakepressed,
  ...
  out = {"follow_brake_rms": None, "follow_brake_mean": None, "follow_gas_rms": None,
         ...
         **uphill_tracking_bin_metrics([], [], [], [], [], [], [], [], [], [], [], [], [], []),
         ...
         "level_positive_sec": None, "level_positive_events": None,
         "level_positive_longest": None, "level_positive_request_median": None,
         "level_positive_aego_median": None, "level_positive_response_error_mean": None,
         "level_positive_response_error_rms": None, "level_positive_wire_rms": None,
  ...
  out.update(level_positive_response_metrics(
    grid, requested, aego, AC, vego, pitch, active, cruise_plan, allow_throttle, has_lead,
    gaspressed, brakepressed,
    speed_min=FOLLOW_MIN_VEGO,
    pitch_abs_max=LEVEL_RESPONSE_PITCH_MAX,
    request_min=LEVEL_RESPONSE_REQUEST_MIN,
    min_episode_s=LEVEL_RESPONSE_MIN_EPISODE_S,
  ))
  ...
  out.update(uphill_near_zero_gas_step_metrics(
    grid, requested, eng_all, vego_all, pitch, BR, brakepressed, GAS,
    speed_min=UPHILL_GAS_STEP_SPEED_MIN,
    pitch_min=UPHILL_GAS_STEP_PITCH_MIN,
    pitch_max=UPHILL_GAS_STEP_PITCH_MAX,
  ...
  out.update(uphill_tracking_bin_metrics(
    ...
    has_lead, gaspressed, brakepressed, BR, GAS,
  ))
...
def verdicts(r):
  ...
  if r.get("lat_direct_sent_frames"):
    paired = r["lat_direct_paired_frames"]
    direct_max = (f"{r['lat_direct_sent_max_abs']:.0f}" if r["lat_direct_sent_max_abs"] is not None else "n/a")
    add("lateral direct TX (diagnostic)", True,
        f"{paired}/{r['lat_direct_sent_frames']} bus-1 sends paired to carOutput; "
        f"{r['lat_direct_extended_frames']} active frames >{ODYSSEY_STOCK_LKA_MAX}, "
        f"active max {direct_max}; "
        + (f"exact {r['lat_direct_exact_frac'] * 100.0:.2f}%, "
           f"MAE {r['lat_direct_mae']:.3f} counts" if paired else "no bounded output pairs"))
  ...
  if r.get("under_set_speed_sec") is not None:
    if r["under_set_speed_events"]:
      add("under-set speed (diagnostic)", True,
          f"{r['under_set_speed_sec']:.2f}s over {r['under_set_speed_events']} event(s), "
          f"longest {r['under_set_speed_longest']:.2f}s; gap "
          f"{r['under_set_speed_gap_median'] * 2.237:.2f} mph, request "
          f"{r['under_set_speed_request_median']:+.3f}, aEgo "
          f"{r['under_set_speed_aego_median']:+.3f}, pitch "
          f"{r['under_set_speed_pitch_median']:+.3f} rad; response error "
          f"{r['under_set_speed_response_error_mean']:+.3f} m/s^2")
    else:
      add("under-set speed (diagnostic)", True,
          "0.00s over 0 event(s); no qualifying sustained no-lead cruise exposure")
  if r.get("level_positive_sec") is not None:
    if r["level_positive_events"]:
      add("near-level positive response (diagnostic)", True,
          f"{r['level_positive_sec']:.2f}s over {r['level_positive_events']} event(s), "
          f"longest {r['level_positive_longest']:.2f}s; request "
          f"{r['level_positive_request_median']:+.3f}, aEgo "
          f"{r['level_positive_aego_median']:+.3f}, pitch "
          f"{r['level_positive_pitch_median']:+.3f} rad; response error "
          f"{r['level_positive_response_error_mean']:+.3f} m/s^2, wire RMS "
          f"{r['level_positive_wire_rms']:.3f}")
    else:
      add("near-level positive response (diagnostic)", True,
          "0.00s over 0 event(s); no qualifying sustained near-level cruise exposure")
  if r.get("cruise_button_press_events") is not None:
    counts = ", ".join(f"{kind}={count}" for kind, count in
                        r.get("cruise_button_press_counts", {}).items()) or "none"
    up = r.get("cruise_set_speed_up_events", 0)
    down = r.get("cruise_set_speed_down_events", 0)
    max_up = r.get("cruise_set_speed_up_max_mps")
    max_down = r.get("cruise_set_speed_down_max_mps")
    speed_detail = f"{up} up/{down} down set-speed change(s)"
    if max_up is not None or max_down is not None:
      speed_detail += f" (largest +{max_up or 0.0:.2f}/-{abs(max_down or 0.0):.2f} m/s)"
    truncated = " [event list truncated]" if r.get("cruise_input_events_truncated") else ""
    add("cruise input (diagnostic)", True,
        f"{r['cruise_button_press_events']} button press(es): {counts}; {speed_detail}{truncated}")
  ...
  if r.get("uphill_near_zero_gas_step_events") is not None:
    median = (f"{r['uphill_near_zero_gas_step_median']:.0f}"
              if r.get("uphill_near_zero_gas_step_median") is not None else "n/a")
    add("uphill near-zero live-gas steps (diagnostic)", True,
  ...
  for band in ("moderate", "high"):
    ...
    if episodes is not None:
      response = r.get(f"uphill_{band}_response_error_median")
      response_text = f"{response:+.3f}" if response is not None else "n/a"
      wire = r.get(f"uphill_{band}_wire_rms")
      wire_text = f"{wire:.3f}" if wire is not None else "n/a"
      add(f"uphill {band} request tracking (diagnostic)", True,
          f"{episodes} episode(s), {r[f'uphill_{band}_sec']:.1f}s settled gas exposure; "
          f"median aEgo-carControl {response_text}, wire-request RMS {wire_text} m/s^2 "
          "(unmatched route readout)")
...
def write_ledger_md(rows):
  ...
  header = (
    ...
    "counts physical BRAKE_REQUEST edges. The ungraded `under-set speed (diagnostic)` exposure "
    "fields and bounded route-relative cruise button/set-speed transitions are retained in the "
    "JSONL.\n\n"
...
def main():
  ...
  quality = {"following - gas domain", "following - brake domain",
             ...
             "under-set speed (diagnostic)",
             "cruise input (diagnostic)",
